"""Fixed R14B stable food-price pipeline replay; --verify performs full offline refits."""
...
from models.food_stable_r14b import MODELS,load_inputs,forecast_origin,replace_food
...
OUT=ROOT/'output/research_r14b/food'
DECLARATION='a3740fe'
REFERENCES=('INDEPENDENT_BRIDGE','FOOD_TREND_R12','FOOD_COST_R12',
            'FOOD_DOMESTIC_PIPELINE_R14','FOOD_OWN_LEVEL_AR_R14')
...
def uncertainty(frame):
    rng=np.random.default_rng(1410); records=[]
        ...
        for h in range(1,13):
            ...
            for name,control in PAIRS:
                ...
                records.append(dict(sample=sample,h=h,model=name,control=control,n=n,mse_difference=d.mean(),
                    ci_low=lo,ci_high=hi,block_length=12,replicates=2000,seed=1410))
...
def dependency_hashes():
    names=['food_stable_experiment_r14b.py','models/food_stable_r14b.py','test_food_stable_r14b.py',
        'food_path_experiment_r14.py','models/food_path_r14.py','test_food_path_r14.py',
        'docs/implementation/R14B_FOOD_DESIGN.md','output/research_r14/food/native_forecasts.csv',
        'output/research_r14/food/manifest.json',
...
def run(destination=OUT):
    ...
    original_manifest=json.loads((ROOT/'output/research_r14/food/manifest.json').read_text())
    for name,digest in original_manifest['outputs'].items():
        if sha(ROOT/'output/research_r14/food'/name)!=digest:
            raise AssertionError(f'Changed original R14 food payload: {name}')
    old_food=pd.read_csv(ROOT/'output/research_r14/food/native_forecasts.csv',float_precision='round_trip')
    r12=pd.concat([r12,old_food],ignore_index=True)
    ...
    for i,(origin,row) in enumerate(h0.iterrows()):
        ...
        diagnostics.append(dict(origin=origin,**result['diagnostics'],forecast_food_rates=result['forecast_food_rates']))
    ...
    manifest=dict(prespec_commit=DECLARATION,completed_at=datetime.now(timezone.utc).isoformat(),inputs=hashes,outputs=outputs,
        command='python food_stable_experiment_r14b.py',models=list(MODELS),primary=MODELS[0],origins=90,
        python=platform.python_version(),packages={p:importlib.metadata.version(p) for p in ('numpy','pandas','scipy','statsmodels')},
        interpretation='Outcome-informed R14B stable monthly-rate adaptation; no structural identification, historical vintages, or fresh holdout claim.')
...
def verify():
    ...
    with tempfile.TemporaryDirectory(prefix='r14b_food_') as folder,ExitStack() as stack:
