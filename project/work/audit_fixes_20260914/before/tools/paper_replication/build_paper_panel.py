...
for month ``e+1`` (CPI for ``e`` is then known).  Nothing uses later data: no
...
Where Table A6's log difference is undefined because a series takes zero or
negative values (rows 22, 25 and the fuel component 64), the first difference
...
def transform(series: pd.Series, code: int) -> tuple[pd.Series, str]:
    if code == 0:
        return series, "T0"
    if code == 3:
    ...
    if code == 2:
        if (series.dropna() <= 0).any():
            return series.diff(), "T2 undefined (non-positive values): T3 used"
        return np.log(series).diff(), "T2"
...
def paper_panel(raws: dict[int, Raw], audit: pd.DataFrame, end: pd.Period = PAPER_END,
    ...
    for row in ROWS:
        ...
        values, entry["transform"] = transform(level, row.transform)
...
def realtime_panel(raws: dict[int, Raw], calendar: Path = CALENDAR, end: pd.Period | None = None) -> tuple[pd.DataFrame, dict]:
    ...
    for row in ROWS:
        ...
        if raw.kind == "components":
            ...
            entry["transform"] = "T2 undefined for a principal component: T3 of the component re-estimated at each row"
            log[n] = entry
            continue
