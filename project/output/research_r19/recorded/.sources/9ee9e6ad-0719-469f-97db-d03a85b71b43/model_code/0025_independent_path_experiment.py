...
python independent_path_experiment.py [--resume] [--long-extra] [--frozen-inputs CSV]
...
from models.trend_gap import fit_forecast, require_monthly_contiguous, seasonal_means
...
def _data_fingerprints(headline, fx):
    """Hash actual monthly arrays, including calendar, missingness and float bits."""
    result = {}
    for name, series in (("headline_mm_array", headline), ("eurczk_array", fx)):
        require_monthly_contiguous(series, name)
        digest = hashlib.sha256()
        digest.update(json.dumps({"frequency": series.index.freqstr,
                                  "periods": series.index.astype(str).tolist()},
                                 separators=(",", ":")).encode("utf-8"))
        digest.update(np.asarray(series.to_numpy(), dtype="<f8").tobytes())
        result[name] = digest.hexdigest()
    return result


def _load_frozen_inputs(path):
    """Read exact CSV float values; trim union padding, never interior gaps."""
    frame = pd.read_csv(path, index_col="period", float_precision="round_trip")
    frame.index = pd.PeriodIndex(frame.index, freq="M")
    require_monthly_contiguous(frame, "frozen monthly inputs")
    series = []
    for name in ("headline_mm", "eurczk"):
        values = frame[name].astype(float)
        if values.first_valid_index() is None:
            raise ValueError(f"Frozen {name} has no finite history")
        series.append(values.loc[values.first_valid_index():values.last_valid_index()])
    return tuple(series)


def _verify_resume(saved, fingerprints):
    """Legacy or changed input/code checkpoints cannot be silently combined."""
    if saved.get("fingerprints") != fingerprints:
        raise ValueError("Resume input/code hashes differ from the saved experiment; start a fresh run")
...
def main(argv=None):
    ...
    parser.add_argument("--frozen-inputs", type=Path,
                        help="Use a saved headline_mm/eurczk CSV instead of database inputs")
    ...
    if args.frozen_inputs is not None:
        y, fx = _load_frozen_inputs(args.frozen_inputs)
    else:
    ...
    reference_path = ROOT / "output" / "path_step2.csv"
    reference = pd.read_csv(reference_path).set_index(["origin", "h"])
    archive = ROOT / "data" / "vintages" / "unemployment.csv.gz"
    files = [Path(__file__), ROOT / "models/path_inputs.py", ROOT / "models/trend_gap.py",
             ROOT / "models/bvar.py", ROOT / "data/vintages.py", ROOT / "cz_struct.py",
             ROOT / "path_experiment.py", ROOT / "data/release_calendar_cz_cpi.csv",
             archive, h0_path, reference_path]
    fingerprints = {p.relative_to(ROOT).as_posix(): _sha(p) for p in files}
    fingerprints.update(_data_fingerprints(y, fx))
    packages = {p: importlib.metadata.version(p) for p in ["numpy", "pandas", "scikit-learn", "statsmodels"]}
    fingerprints["runtime_packages"] = hashlib.sha256(json.dumps(packages, sort_keys=True).encode()).hexdigest()
    ...
    if args.resume:
        if not forecast_path.exists() or not manifest_path.exists():
            raise ValueError("Resume requires both a saved forecast checkpoint and its input/code hashes")
        saved = json.loads(manifest_path.read_text(encoding="utf-8"))
        _verify_resume(saved, fingerprints)
        if saved.get("origins_requested") != [str(t) for t in origins]:
            raise ValueError("Resume origin schedule differs from the saved experiment")
        previous = pd.read_csv(forecast_path, float_precision="round_trip")
    ...
    manifest = dict(started_at=datetime.now(timezone.utc).isoformat(), fingerprints=fingerprints,
        ...
        rf_options=RF_OPTIONS, future_scenario="ML only: FX last completed monthly level held; unpublished U changes zero. Direct BVAR/RF use history-edge predictors, without future scenario rows",
        primary_data_source="frozen_csv" if args.frozen_inputs is not None else "database",
