...
def load_path_inputs(folder):
    ...
    for name,digest in manifest['files'].items():
        if sha(folder/name)!=digest:raise ValueError('Path input hash mismatch: '+name)
    levels=read_monthly(folder/PATH_FILES[0]);available=read_monthly(folder/PATH_FILES[1])
    ...
    pump=pd.read_csv(folder/PATH_FILES[2],index_col=0,parse_dates=True,float_precision='round_trip')
...
def input_freshness(levels,available,target,as_of):
    ...
    for col in ('agri4','food_ppi','food'):
        ...
        edge=max(usable) if usable else None
        stale=expected is None or edge is None or edge<expected
        rows.append(dict(series=col,expected_released_month=str(expected),latest_released_month=str(edge),stale=stale))
...
def main(argv=None):
    ...
    print(json.dumps({'run':str(destination),'readiness':json.loads((destination/'readiness.json').read_text())},indent=2))
