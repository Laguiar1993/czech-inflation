...
**Codex audit addendum, 14 September 2026:** corrections are implemented in this working tree. Start with `IMPLEMENTED_AUDIT_FIXES_2026-09-14.md` and the earlier `REVIEW_TO_CLAUDE_2026-09-14.md`. Saved historical tables below retain their original generating specification unless explicitly corrected. New causal TVW results live in `output/audit_fixes_20260914/causal_tvw_common/`; they supersede the old TVW calibration for future runs. Paper benchmark proximity is an approximation, not verified replication; `_TMH` has a different information cutoff and the `yoy6` statistic mixes origins. The basket-hour issue is out of scope at the user's request.
...
- research: CNB WP 9/2026-inspired TVW-QRF on the 72-variable A6 panel (approximate monthly comparison; exact paper replication is not established, and path promotion has failed).
...
Reading:
- HARD_BASE loses to consensus over the whole sample. Large energy-policy misses matter, but the earlier 83% attribution was incorrect: the three largest squared errors account for 57.3% of total SSE; the January-2022/October-2022/January-2023 policy trio accounts for 50.2%. These are different selections of months. Since 2024 its RMSE is lower than consensus in the listed frames, including about 11% lower in the flash era. This does not establish statistical superiority or prospective performance.
...
**TVW-QRF lane verdict, qualified by Codex** (`docs/implementation/PAPER_TVWQRF_SPEC_2026-09-12.md`): the archived monthly RMSEs approximate the published results. Exact replication is not established: some input and algorithm choices differ, the `_TMH` benchmarks use t−h while the forest uses t, and historical TVW calibration has now been replaced. The old TVW3 0.707 / 0.695 / 0.714 / 0.650 table remains historical evidence, not the corrected model's result. Path promotion still fails after the correction. The unchanged QRF mean with calendar month remains interesting in recent CNB-report comparisons (0.409 vs CNB 0.372 vs bridge 0.590), with small, overlapping, post-hoc samples. User-approved LUCI/Rushin inputs retain revised-history risk; approval to use them is not evidence of real-time vintages.
