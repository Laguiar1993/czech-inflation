...
def is_big_surprise(surprise):
    """Inclusive 0.4 pp threshold, robust to decimal floating-point subtraction."""
    return abs(surprise) >= .4 - SCORE_EPS
...
def score_forecasts(predictions, output_dir=None):
    ...
    for model, p in forecasts.items():
        ...
        ev = pd.DataFrame(dict(model=model, actual=sv.actual, consensus=sv.survey_median,
             ...
             big=is_big_surprise(surprise), alert=d.abs() >= .2-SCORE_EPS))
        ev['directional_big_hit'] = ev.alert & ev.big & (d * surprise > 0)
        ev['wrong_way_big_alert'] = ev.alert & ev.big & (d * surprise < 0)
        ev['material_big_hit'] = ev.alert & ev.big & (gain >= .15-SCORE_EPS)
        ...
        for frame, mask in [('all', p.notna()), ('ex_jan', p.index.month != 1),
            ...
            scores.append(dict(model=model, frame=frame, n=len(z),
                ...
                closer=int((z.gain > SCORE_EPS).sum()),
                material_win=int((z.gain >= .15-SCORE_EPS).sum()),
                material_loss=int((z.gain <= -.15+SCORE_EPS).sum()),
                ...
                directional_big_hit=int(z.directional_big_hit.sum()),
                wrong_way_big_alert=int(z.wrong_way_big_alert.sum()),
                material_big_hit=int(z.material_big_hit.sum()),
                directional_big_precision=float(z.directional_big_hit.sum()/z.alert.sum()) if z.alert.sum() else np.nan,
                material_big_precision=float(z.material_big_hit.sum()/z.alert.sum()) if z.alert.sum() else np.nan))
    pd.concat(events).to_csv(output_dir/'independent_nowcast_releases.csv', index_label='period')
    board = pd.DataFrame(scores)
    board.to_csv(output_dir/'independent_nowcast_scores.csv', index=False)
    (output_dir/'independent_nowcast_score_definitions.json').write_text(json.dumps({
        'alert': 'absolute forecast minus consensus >= 0.2 pp',
        'big': 'absolute actual minus consensus >= 0.4 pp',
        'threshold_epsilon': SCORE_EPS,
        'alerted_big': 'magnitude detection only: alert and big, irrespective of direction or gain',
        'false_alarm': 'alert on a non-big release; not necessarily a harmful forecast',
        'directional_big_hit': 'alert and big and forecast deviation has the surprise direction',
        'wrong_way_big_alert': 'alert and big and forecast deviation has the opposite direction',
        'material_big_hit': 'alert and big and absolute-error gain >= 0.15 pp',
        'big_precision': 'magnitude big hits / all alerts in the selected frame',
        'directional_big_precision': 'directional big hits / all alerts in the selected frame',
        'material_big_precision': 'material big hits / all alerts in the selected frame',
        'all_release_precision': 'Use frame=all for all-release alert precision. Frame=big is conditional on realised large surprises and is not all-release precision.'
    }, indent=2))
    ...
    (output_dir/'independent_nowcast_selection.json').write_text(json.dumps(decision, indent=2))
    ...
    pd.DataFrame(boot).to_csv(output_dir/'independent_nowcast_bootstrap.csv',index=False)
...
if __name__=='__main__':
    ...
    parser.add_argument('--score-only', type=Path, help='Rescore saved predictions without fitting or changing them')
    parser.add_argument('--output-dir', type=Path, help='Required separate directory for --score-only outputs')
    args = parser.parse_args()
    if args.score_only is not None:
        if args.output_dir is None or args.output_dir.resolve() == OUT.resolve():
            parser.error('--score-only requires a separate --output-dir to preserve original scores')
        pred = pd.read_csv(args.score_only, index_col='period', float_precision='round_trip')
        pred.index = pd.PeriodIndex(pred.index, freq='M')
        board, decision = score_forecasts(pred, output_dir=args.output_dir)
        print('All-release alert precision: numerator is directional/material big hits; denominator is every alert.')
        print(board[board.frame.eq('all')].to_string(index=False))
        print(decision)
    elif args.output_dir is not None:
        parser.error('--output-dir is supported with --score-only')
    else:
        main()
