...
from models.nowcast_reliability_r18_final import weighted_quantile,crps
...
def main():
    ...
    p.add_argument('--output',type=Path,default=ROOT/'output/research_r18/uncertainty_final');args=p.parse_args()
    out=args.output;out.mkdir(parents=True,exist_ok=False);source=args.experiment/'native_forecasts.csv'
    calendar_source=args.experiment/'evaluation/primary_rows.csv'
    hashes=c.preserved_hashes();names=[source,ROOT/'models/path_uncertainty_r18.py',ROOT/'models/nowcast_reliability_r18.py',ROOT/'models/nowcast_reliability_r18_final.py',
        Path(__file__),ROOT/'docs/implementation/R18_UNCERTAINTY_SPEC_2026-09-15.md',ROOT/'data/release_calendar_cz_cpi.csv',
        ROOT/'output/independent_path_frozen_inputs.csv',calendar_source]
    ...
    calendar=pd.read_csv(calendar_source,usecols=['origin','h']).drop_duplicates()
    if len(calendar)!=969:
        raise ValueError('Expected original 969 scored origin/horizon keys')
    intended=intended.merge(calendar,on=['origin','h'],how='inner',validate='many_to_one')
    ...
    c.finish(out,hashes,models=models,probability_status='experimental_uncalibrated',h0_errors_included=True,
             scored_calendar='original_969_keys_then_common_available_intervals')
