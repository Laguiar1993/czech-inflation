...
def inspect_run(run,*,mode,root=ROOT):
    ...
    # Keep every receipt-bound output, including optional legacy path files.
    # They are evidence only: path_forecasts remains empty in the new bundle.
    manifest_paths=[run/'receipt.json']+[run/name for name in receipt]
...
def record_run(run,archive_root,*,mode,root=ROOT):
    ...
    metadata.update(units={'point':'monthly_CPI_percent','contributions':'percentage_points'},
        ...
        freshness_policy={'completion_age_limit_at_handoff_seconds':120,
            'archive_issue_to_commit_limit_seconds':archive.NOW_WINDOW_SECONDS,
            'handoff_at_utc':observed.isoformat(),
            'note':'Two separate clocks; the completion-to-commit limit can total 240 seconds. All prospective publication must precede first release.'},
