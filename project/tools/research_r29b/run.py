"""R29B: level-based upstream phase; fixed persistence pairs as candidates plus the panel-estimated pair.

Run from the repository root into a directory that does not exist yet:
    python -m tools.research_r29b.run --output output/research_r29b/final [--frame-run output/research_r27/final --frame-model FOOD_ECM_R27]
...
from models.core_phase_r29 import TARGET_LOG_PER_MONTH, band_corrections, lambda_phase_at
from models.core_phase_r29b import MODELS, FIXED_PAIRS, PANEL_MODEL, phase_at

SPEC = 'docs/implementation/R29B_CORE_PHASE_LEVEL_SPEC_2026-09-18.md'
R25 = 'output/research_r25/final'
CODE = ['models/core_phase_r29b.py', 'models/core_phase_r29.py', 'models/panel_persistence_r25.py', 'models/cost_gaps_r23.py', 'data/ppi_panel_r29.py', 'data/cost_gaps_r23.py', 'r17_common.py', 'tools/research_r29b/run.py']
...
def main():
    ...
    phases = pd.DataFrame(phases); panel = panel.merge(phases[['geo', 'origin', 'phase', 'momentum', 'last_month']], on=['geo', 'origin'], how='left')
    print(f'R29B panel: {len(panel)} rows, phase known for {panel.phase.notna().mean():.1%}; building share {(panel.phase.eq("building")).sum() / max(panel.phase.notna().sum(), 1):.2f}, {time.perf_counter() - start:.1f}s', flush=True)
    ...
    fixed = dict(FIXED_PAIRS); all_models = list(MODELS)
    ...
    with (out / 'fits.jsonl').open('w', encoding='utf-8') as log:
        for i, origin in enumerate(origins):
            ...
            chosen = {}
            if status == 'estimated':
                chosen[PANEL_MODEL] = ([d['lam'] for d in lam_phase[cz_phase]], TARGET_LOG_PER_MONTH)
            ...
            if i % 10 == 0 or i == len(origins) - 1:
                print(f'R29B {i + 1}/{len(origins)} {origin}: phase={cz_phase} lam_b={np.round(record["lambda_building"], 2).tolist()} lam_f={np.round(record["lambda_fading"], 2).tolist()}, {time.perf_counter() - start:.1f}s', flush=True)
    ...
    c.finish(out, hashes, controls=controls, models=all_models, origin_count=len(origins), frame_run=args.frame_run, frame_model=args.frame_model,
             ...
             notes='R29B level-based upstream phase on the roster frame; fixed persistence pairs (1.0 building; 0.4/0.6/0.8 fading) and the panel pair; only the core block of h1-12 changes.')
    m = json.loads((out / 'manifest.json').read_text()); m['outputs']['fits.jsonl'] = c.sha(out / 'fits.jsonl'); c.dump(out / 'manifest.json', m)
    print('Completed R29B', out, flush=True)
