"""R29B scoring: the level-based phase; fixed pairs as candidates, the panel pair gated; the six conditions.

    python -m tools.research_r29b.evaluate --root output/research_r29b/final
"""
import argparse
import json
...
from models.core_phase_r29b import MODELS, FIXED_PAIRS, PANEL_MODEL
from tools.path_diagnostics import bootstrap, gates, standard

BLOCK = 'core'; REFERENCE = 'STATE_FAST_R15'
FIXED = dict(FIXED_PAIRS)
LABELS = {PANEL_MODEL: 'Core: level phase, panel weights, 2% norm', **{k: f'Core: level phase, fixed weights 1.0 / {v:.1f}, 2% norm' for k, v in FIXED.items()}}
METHOD = [
    'R29B: core persistence conditioned on the level of upstream producer-price momentum (building above 2% a year), on the research roster frame, original 90 origins and 969 primary keys. Only the core block of h1-12 changes. Historical research, not a live record.',
    'The phase at an origin is read from the last published producer-price month: six-month momentum above 1.0 log point (2% a year) is building, otherwise fading. Fixed weight pairs (1.0 building; 0.4, 0.6, 0.8 fading) are candidates; the panel pair uses pooled slopes on the frozen R25 EU-panel rows split by the same phase and is scored only if the panel separates the phases.',
...
def main():
    ...
    out, _ = standard.standard_evaluation(root, 'R29B', LABELS, METHOD, defaults={frame_model, 'CORE_LEVELPHASE_FIXED080_R29B'})
    ...
    if not gate['passes']:
        verdicts[PANEL_MODEL]['promotable'] = False; verdicts[PANEL_MODEL]['failed'] = ['panel_gate', *verdicts[PANEL_MODEL]['failed']]
    passing = [m for m in FIXED if verdicts[m]['promotable']]
    pivot = score.set_index(['name', 'H', 'sample']).rmse; cells = [(H, s) for H in SCORE_H for s in standard.ERAS]
    balanced = {m: float(np.mean([pivot.get((m, H, s), np.nan) / pivot.get((REFERENCE, H, s), np.nan) for H, s in cells])) for m in FIXED}
    c.dump(out / 'fixed_pair_choice.json', dict(passing=passing, era_balanced_score=balanced, chosen=min(passing, key=balanced.get) if passing else None))
    c.dump(out / 'rule_verdicts.json', verdicts)
    standard.close_manifest(out, extra_code=[Path(__file__), ROOT / 'models/benchmarks_r26.py', ROOT / 'models/core_phase_r29.py', ROOT / 'models/core_phase_r29b.py'])
    ...
    print('Completed R29B evaluation', out, flush=True)
