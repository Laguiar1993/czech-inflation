...
# Extra-lag diagnostics approximate the published benchmark RMSEs, but their
# information ends at t-h while the forest uses t. This is not a fair comparison
# or proof of the paper authors' generating specification.
...
def forecast_task(edge: str, horizon: int, panel: pd.DataFrame, target: pd.Series, convention: str,
                  forest: dict, lqr: bool, include_month: bool = False, warmup: bool = False) -> dict:
    start = time.perf_counter()
    t = pd.Period(edge, freq="M")
    X, y, x_now, rows, columns = training_arrays(panel, target, t, horizon, convention, include_month)
    if warmup and len(y) < engine.VALIDATION_WINDOW + 24:
        return {'points': [], 'weights': [], 'quantiles': [],
                'diagnostics': dict(edge=edge, horizon=horizon, n_train=len(y),
                                    n_features=len(columns), seconds=time.perf_counter()-start,
                                    status='skipped_short_warmup')}
...
def forecast_tasks(edges, horizons, last_target, panel_end, panel_start=TRAIN_START):
    """Schedule own-origin distributions, including mature calibration warmup."""
    tasks = []
    if len(horizons) != len(set(horizons)):
        raise ValueError('Duplicate horizons are not permitted')
    for h in horizons:
        if not isinstance(h, int) or h < 1:
            raise ValueError('Positive integer horizons required')
        wanted = sorted({e for e in edges if e+h <= last_target and e <= panel_end})
        if not wanted:
            continue
        first = max(wanted[0]-h-engine.VALIDATION_WINDOW+1,
                    max(TRAIN_START, panel_start)+h+engine.VALIDATION_WINDOW+23)
        if first > wanted[0]:
            raise ValueError('First requested origin lacks minimum training history')
        tasks.extend((str(t), h) for t in pd.period_range(first, wanted[-1], freq='M'))
    return sorted(tasks)


def benchmarks_for_tasks(target, tasks, first_edge, tmh=False):
    """Give benchmarks exactly the scored forest origin/horizon keys."""
    by_edge = {}
    for edge, h in tasks:
        t = pd.Period(edge, freq='M')
        if t >= first_edge:
            by_edge.setdefault(t, set()).add(h)
    return [row for edge, horizons in sorted(by_edge.items())
            for row in benchmark_rows(target, [edge], sorted(horizons), tmh)]
...
def yoy6(forecasts: pd.DataFrame, target: pd.Series) -> pd.DataFrame:
    """Mixed-origin diagnostic: six h6 forecasts issued at six different origins.

    This is not one origin's six-month-ahead path and does not establish the
    paper's Table A4/A5 generating specification. Use paper_tvwqrf_path for
    same-origin path evaluation.
    """
...
def main(argv=None) -> int:
    ...
    parser.add_argument("--tmh-benchmarks", action="store_true",
                        help="also run extra-lag benchmark diagnostics (not a common-information comparison or verified paper replication)")
    ...
    if args.limit_edges is not None and args.limit_edges < 1:
        parser.error('--limit-edges must be positive')
    ...
    tasks = forecast_tasks(edges, horizons, last_target, panel.index.max(), panel.index.min())
    if not tasks:
        raise ValueError('No forecast tasks in the requested window')
    forest = {**engine.FOREST_DEFAULTS, **json.loads(args.forest)}
    output = Path(args.output)
    output.mkdir(parents=True, exist_ok=False)
    started = time.time()
    results = Parallel(n_jobs=args.jobs, verbose=5)(
        delayed(forecast_task)(edge, h, panel, target, args.convention, forest, args.lqr,
                               args.include_month, pd.Period(edge, freq='M') < first_edge)
        for edge, h in tasks)
    quantiles = pd.DataFrame([r for res in results for r in res["quantiles"]])
    calibration = engine.recalibrate_saved(quantiles, target)
    points = pd.DataFrame([r for res in results for r in res["points"]])
    points = pd.concat([points[~points.model.isin(engine.SCHEMES)], calibration['points']], ignore_index=True)
    points = pd.concat([points, pd.DataFrame(benchmarks_for_tasks(target, tasks, first_edge, args.tmh_benchmarks))], ignore_index=True)
    points["edge"] = pd.PeriodIndex(points.edge, freq="M")
    points["target_period"] = points.edge + points.horizon
    points = points[(points.edge >= first_edge) & (points.target_period <= last_target)]
    ...
    calibration['weights'].to_csv(output / "tvw_weights.csv", index=False)
    calibration['diagnostics'].to_csv(output / "calibration.csv", index=False)
    ...
    manifest = {
        ...
        "edges": [str(points.edge.min()), str(points.edge.max())], "tasks": len(tasks), "last_target": str(last_target),
        ...
        "calibration": "saved own-origin quantiles; twelve consecutive matured targets; fixed feasible weights if incomplete",
        "first_scheduled_edge_including_warmup": min(e for e, _ in tasks),
        "first_distribution_edge_including_warmup": quantiles.edge.min(),
        "quantiles_include_warmup": True,
