...
implementation and numerical runtime. Only output/forest_tuning_* artifacts are written.
"""
...
def _runtime_identity():
    return {"python": platform.python_version(),
            **{name: importlib.metadata.version(name)
               for name in ("numpy", "pandas", "scipy", "quantile-forest", "scikit-learn")}}
...
def forest_prediction(features: pd.DataFrame, errors: pd.Series, origin: pd.Period,
    ...
    key = hashlib.sha256(f"{input_hash}|{parameter_hash}|{source_hash}|{runtime_hash}".encode()).hexdigest()
    info = {"origin": origin, "as_of": as_of, "forest": config.key, "correction": 0.0,
            ...
            "parameters_sha256": parameter_hash, "source_sha256": source_hash,
            "runtime": runtime, "runtime_sha256": runtime_hash, "cache_hit": False}
    cache_file = None
    if cache_dir is not None:
        ...
        if cache_file.exists():
            ...
            if (saved.get("cache_key") != key or saved.get("input_sha256") != input_hash
                    or saved.get("parameters_sha256") != parameter_hash or saved.get("source_sha256") != source_hash
                    or saved.get("runtime_sha256") != runtime_hash or saved.get("runtime") != runtime):
                raise ValueError("forest cache fingerprint mismatch")
            try:
                correction = float(saved["correction"])
            except (KeyError, TypeError, ValueError) as exc:
                raise ValueError("forest cache correction must be finite") from exc
            if not np.isfinite(correction):
                raise ValueError("nonfinite forest cache correction")
