...
## Offline operator package (R48e, 25 September 2026) — start here on the Bloomberg computer

Download **[czech-inflation-offline-r48e.zip](https://github.com/Laguiar1993/czech-inflation/releases/download/r48e-offline-2026-09-25/czech-inflation-offline-r48e.zip)** from release [r48e-offline-2026-09-25](https://github.com/Laguiar1993/czech-inflation/releases/tag/r48e-offline-2026-09-25): 151,285,775 bytes, SHA-256 `3142e98d9fa1f029451d55bfb5d20c8cd6ad411601256739253beb877fe8fab5`, built from commit `08140a5` (3,600 members; `PACKAGE_MANIFEST.json` inside lists every member hash).

Check the file you downloaded says **r48e**. Each release is now named for its own version; the folder inside every archive stays `czech-inflation-offline-r48`, because the extraction and verification commands check for that name.
...
Extract it, read `START_HERE.txt`, then **`OFFLINE_OPERATOR_GUIDE.md`**, and keep **[MANUAL_DOWNLOADS.md](MANUAL_DOWNLOADS.md)** beside you for the inputs the Terminal does not carry.

New in R48e: exactly one page folder travels and `START_HERE.txt` names it, so there is no way to open a superseded page and read a forecast that has since moved. New in R48d: `cpi ledger show` answers "what is out of date" from evidence rather than a rule of thumb, printing for every series the last figure held, the day it actually reached us, how old that is and how often the series delivers, and calling a series overdue only when its own rhythm says a newer figure should already be in hand; the record is append-only, so checking daily never makes anything look fresher than it is. `MANUAL_DOWNLOADS.md` gives the URL, commands and checks for the four by-hand inputs.

Earlier in the R48 line: the 37-group blocks build on the staged download and advance with everything else, and every figure is labelled with the month its own source reaches (R48c); the freshness alert banner on the page, and farm prices admitted from the 18th under versioned availability rules (R48b); the page rebuilt from your runs with a computed overview and no frozen text, the three-model roster, live CNB rounds, the Bloomberg release calendar, and the Python 3.10 lock (numpy 2.2.4, pandas 2.2.3, scipy 1.15.2, statsmodels 0.14.4, scikit-learn 1.6.1) on which `portable verify --models` reproduces the main nowcast and the path exactly (R48). Works with Python 3.10, 3.11 or 3.12; no command calls the CNB ARAD API or any AI service.

## Previous offline packages

Releases [r48d](https://github.com/Laguiar1993/czech-inflation/releases/tag/r48d-offline-2026-09-25), [r48c](https://github.com/Laguiar1993/czech-inflation/releases/tag/r48c-offline-2026-09-25), [r48b](https://github.com/Laguiar1993/czech-inflation/releases/tag/r48b-offline-2026-09-25) and [r48](https://github.com/Laguiar1993/czech-inflation/releases/tag/r48-offline-2026-09-24) are the same route at earlier stages; r48d and earlier ship several page folders, of which the oldest two carry the September forecast before the farm-price rule changed. Use R48e above.
